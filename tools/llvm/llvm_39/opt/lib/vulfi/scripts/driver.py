...
def compareNumfiles(file1,file2,threshold):
	...
	if l2norm > threshold:
		...
		return [False,l2norm]
	elif l2norm != l2norm or l2norm == float('Inf') or l2norm == -float('Inf'):
		return [False,-1.0]
	f1.close()
	f2.close()
	return [True,l2norm]
...
def writeResult(arglist,result,diffval,fsCount,detectCount):
	...
	if os.path.exists(arglist.rslt):
		...
		#write header and result
		frslt = open(arglist.rslt,'w')
		frslt.write("exec_key,instr_list,original_val,\
		corrupted_val,affected_bit_pos,sdc,benign,crash,error,value_diff\
		,dyn_instr_count,detection_count\n")
		frslt.write(outline)
		frslt.close()
	return 
